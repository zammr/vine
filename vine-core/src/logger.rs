...
pub fn init_logger(config: &Context) -> Result<(), Error> {
    ...
    let raw_config_value = config.get::<serde_json::Value>("logging")
        .unwrap_or_else(|_| serde_json::from_str::<serde_json::Value>(DEFAULT_LOGGING_CONFIG).unwrap());

    let raw_config = RawConfig::deserialize(raw_config_value).unwrap_or_else(|_| {
        RawConfig::default()
    });

    log4rs::init_raw_config(raw_config).map_err(|_| {
        Error::from(r#"
